...
interface LessonDraft {
  id: string;                 // "l-..." = local; anything else = server id
  title: string;
  description?: string;
  type: LessonType;
  videoUrl?: string;
...
const AdminCourseForm = ({ mode }: Props) => {
  ...
  // Instructor routes are already wrapped by <InstructorLayout> (Outlet-based),
  // so we render without an extra layout in that case.
  const Shell = ({ children }: { children: ReactNode }) =>
    isInstructor ? <>{children}</> : <AdminLayout>{children}</AdminLayout>;
  ...
  // The persisted course id (server-side). In edit mode = route param.
  const [serverCourseId, setServerCourseId] = useState<string | null>(mode === "edit" ? id ?? null : null);
  ...
  // salePrice = what the user pays  → basePrice
  // mrp       = strike-through MRP  → strikeOutPrice
  ...
  useEffect(() => {
    (async () => {
      ...
      if (isAdmin) {
        try {
          const list = await instructorsService.list();
          setInstructors(list);
        } catch {
  ...
  // Course responses store the instructor's auth user id, while the dropdown
  // continues to use the instructor profile id as its option value.
  useEffect(() => {
    if (!basic.instructorId || instructors.some((i) => i.id === basic.instructorId)) return;

    const instructor = instructors.find((i) => i.userId === basic.instructorId);
    if (instructor) {
      setBasic((current) => ({ ...current, instructorId: instructor.id }));
    }
  }, [basic.instructorId, instructors]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mods.length) {
          setModules(mods.map((m: any, i: number) => ({
            id: String(m.id),
            title: m.title || "",
            expanded: false,
            order: m.order ?? i,
            ...
            quiz: (() => {
              const rawQ = m.quiz ?? m.moduleQuiz ?? (Array.isArray(m.quizzes) ? m.quizzes[0] : null);
              const qId = typeof rawQ === "string" ? rawQ : (typeof rawQ === "object" && rawQ?.id) ? String(rawQ.id) : String(m.quizId ?? m.quiz_id ?? "");
              return qId ? {
                id: qId,
                title: (typeof rawQ === "object" && rawQ?.title) || `${m.title || "Module"} Quiz`,
                questionCount: (typeof rawQ === "object" && (rawQ?.questionCount ?? rawQ?.questions?.length)) || 10,
              } : undefined;
            })(),
          })));
        }
  ...
  const validateStep = (s: number): string | null => {
    if (s === 1) {
      ...
      if (isAdmin && !basic.instructorId) return "Please select an instructor";
    }
    if (s === 2) {
  ...
  const buildCoursePayload = () => {
    ...
    if (isAdmin && basic.instructorId) {
      const instructor = instructors.find(
        (i) => i.id === basic.instructorId || i.userId === basic.instructorId,
      );
      if (!instructor) throw new Error("Selected instructor could not be mapped to a user");
      payload.instructorId = instructor.userId;
    }
    return payload;
  };

  // ---------- Step 1 -> 2 : persist the course, upload thumbnail ----------
  ...
  // ---------- Step 2 -> 3 : persist modules + lessons (+ uploads) ----------
 ...
 // Quiz (basic flow only — CSV upload optional, full quiz logic left for later)
   // Quiz
...
if (m.quiz) {
  ...
  const fileToUpload = m.quiz.excelFile || (m.quiz as any).csvFile;
  ...
  // if (m.quiz.excelFile) {
  //   try {
  //     await coursesService.uploadQuizExcel(
  //       quizId,
  //       m.quiz.excelFile
  //     );
  //   } catch {
  ...
  nextQuiz = {
    ...
    excelFile: undefined,
      ...
      //   if (m.quiz.excelFile) {
      //     try { await coursesService.uploadQuizExcel(quizId, m.quiz.excelFile); } catch { /* leave for later */ }
      //   }
      //   nextQuiz = { ...m.quiz, id: quizId, excelFile: undefined };
      // }
  ...
  const openAddQuiz = (mid: string) => {
    const mod = modules.find((m) => m.id === mid);
    setQuizForm(mod?.quiz ?? { id: uid("q"), title: `Quiz for module`, questionCount: 0 });
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      if (!serverCourseId) {
        // Safety net: if user somehow lands on step 4 without a course id
        const cid = await persistCourseFromStep1();
      ...
      toast({ title: publishMode === "publish" ? "Course published" : "Saved as draft" });
      navigate("/admin/courses");
    } catch (e: any) {
  ...
  return (
    <AdminLayout>
      <div className="px-4 sm:px-6 lg:px-8 py-6 max-w-6xl mx-auto">
        ...
        <h1 className="text-2xl font-bold">{mode === "create" && !serverCourseId ? "Create New Course" : "Edit Course"}</h1>
                ...
                <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-5 space-y-4">
                  ...
                  {isAdmin && (
                    <Field label="Instructor" required>
                      <Select value={basic.instructorId} onValueChange={(v) => setBasic({ ...basic, instructorId: v })}>
                        <SelectTrigger>
                          <SelectValue placeholder={instructors.length ? "Assign an instructor..." : "Loading instructors..."} />
                        </SelectTrigger>
                        <SelectContent>
                          {instructors.map((i) => (
                      ...
                      {m.quiz && (
                        <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50/40 border-b border-slate-100">
                          ...
                          <span className="text-[11px] text-muted-foreground">
                            {m.quiz.excelFileName ? m.quiz.excelFileName : "no file"}
                          </span>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {isAdmin && <ChecklistItem item="Instructor assigned" done={!!basic.instructorId} />}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {isAdmin && (
                  <SummaryBox label="Instructor"
                    value={instructors.find(i => i.id === basic.instructorId)?.name || "—"} />
                )}
        ...
        <div className="mt-6 flex items-center justify-between pt-5 border-t border-slate-200">
          {step === 1
            ? <Button variant="outline" onClick={() => navigate("/admin/courses")} disabled={submitting}>Cancel</Button>
...
>
  ...
  <DialogContent className="w-[calc(100%-2rem)] max-w-xl max-h-[90vh] overflow-y-auto p-4 sm:p-6 bg-white rounded-2xl">
    <DialogHeader className="space-y-1 text-left">
      <DialogTitle className="text-lg sm:text-xl font-bold text-slate-900">Quiz</DialogTitle>
      <p className="text-xs text-slate-500 font-normal mt-0.5">
        Upload a CSV to link multiple quizzes
      </p>
    </DialogHeader>
      ...
      <div className="space-y-2">
        ...
        <div className="bg-blue-50/80 border border-blue-100 rounded-xl p-3 sm:p-3.5 space-y-1">
          ...
          <p className="text-xs text-slate-600 leading-relaxed break-words">
            Topic, Question, Option A, B, C, D, Correct Answer, Explanation, Difficulty Level, Is Active
          </p>
        </div>
  ...
  onClick={() => {
    const csv = `Topic,Question,Question Type,Option A,Option B,Option C,Option D,Correct Answers,Difficulty,Explanation
reactjs,What is React?,SINGLE_CHOICE,A JavaScript library,A JavaScript framework,A programming language,A database,A,EASY,React is a JavaScript library for building user interfaces.
hooks js,Which keyword is used to inherit a class in Java?,SINGLE_CHOICE,extends,implements,inherit,super,A,EASY,extends is used for class inheritance.
java spring,Which of the following are primitive data types in Java?,MULTIPLE_CHOICE,int,String,boolean,float,"A,C,D",EASY,String is not a primitive data type.
html,Which principle hides implementation details?,SINGLE_CHOICE,Encapsulation,Polymorphism,Inheritance,Abstraction,A,EASY,Encapsulation hides implementation details.
css,Which keyword refers to the current object in Java?,SINGLE_CHOICE,this,super,self,current,A,EASY,this refers to the current object.
javascript,Which interface does ArrayList implement?,SINGLE_CHOICE,List,Set,Queue,Map,A,MEDIUM,ArrayList implements the List interface.
react hooks,Which collection stores unique values in Java?,SINGLE_CHOICE,Set,List,Queue,ArrayList,A,MEDIUM,Set stores unique values.
hooks,Which methods are available in List?,MULTIPLE_CHOICE,add(),get(),push(),remove(),"A,B,D",MEDIUM,push() belongs to Stack or JavaScript arrays and is not a List method.
hooksddss,Which block always executes?,SINGLE_CHOICE,finally,catch,throw,try,A,MEDIUM,finally executes regardless of exceptions.`;
  ...
  Download Sample CSV
      ...
      <div className="space-y-1.5">
        ...
        <div className="relative w-full">
          <label className="flex items-center w-full min-w-0 h-10 sm:h-11 px-3 border border-slate-200 rounded-lg cursor-pointer bg-white hover:bg-slate-50 text-sm text-slate-600">
            <span className="truncate min-w-0">
              {quizForm.csvFileName ||
                quizForm.excelFileName ||
      ...
      <div className="bg-blue-50/60 border border-blue-100/80 rounded-xl p-3.5 sm:p-4 space-y-3.5 sm:space-y-4">
        <Field label="Quiz Title" required>
          <Input
            className="bg-white border-slate-200 text-sm h-10 w-full"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-x-4 sm:gap-y-3">
          <Field label="Max Attempts :">
            <Input
              type="number"
              min={0}
              className="bg-white border-slate-200 text-sm h-10 sm:h-9 w-full"
    ...
    <div className="grid grid-cols-2 gap-3 pt-2">
      <Button
        type="button"
        variant="outline"
        className="h-10 sm:h-11 w-full border-slate-200 text-slate-700 hover:bg-slate-50 font-medium rounded-lg"
      ...
      <Button
        type="button"
        className="h-10 sm:h-11 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg"
        onClick={saveQuiz}
      >
        Process XLSX
      </Button>
      ...
      {/* <Dialog open={quizDialog.open} onOpenChange={(o) => setQuizDialog({ open: o })}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <p className="text-xs text-muted-foreground">Upload a CSV/XLSX to bulk-import quiz questions.</p>
          </DialogHeader>
          <div className="space-y-3">
            <Field label="Quiz Title" required>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-md p-3">
              <p className="text-xs font-semibold">Quiz Questions upload column template*</p>
              <p className="text-xs text-muted-foreground mt-1">
                Topic, Question, Option A, B, C, D, Correct Answer, Explanation, Difficulty Level, Is Active
              </p>
            </div>
            <Button className="w-full bg-blue-600 hover:bg-blue-700 gap-2" onClick={() => {
              const csv = "Topic,Question,Option A,Option B,Option C,Option D,Correct Answer,Explanation,Difficulty Level,Is Active\nReact,What is React?,A library,A framework,A language,A database,A,React is a JS library for UIs,Easy,Yes\n";
            ...
            <Field label="Upload CSV / XLSX">
              <Input type="file" accept=".csv,.xlsx,.xls"
                onChange={(e) => {
                  const f = e.target.files?.[0];
                  if (f) setQuizForm({ ...quizForm, excelFile: f, excelFileName: f.name });
                }} />
            </Field>
          </div>
          <div className="flex gap-2 pt-2">
            <Button variant="outline" className="flex-1" onClick={() => setQuizDialog({ open: false })}>Cancel</Button>
            <Button className="flex-1 bg-blue-600 hover:bg-blue-700" onClick={saveQuiz}>Save Quiz</Button>
          </div>
        </DialogContent>
      </Dialog> */}
    </AdminLayout>
